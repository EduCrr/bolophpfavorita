...
class EstilosController extends AppController
{
	...
	public function adicionar() {
		...
		if ($this->request->is('ajax')) {
			...
			$estilo->imagem = md5(uniqid((string) rand(), true)) . '.' . strtolower(pathinfo($this->request->getData('img')->getClientFilename(), PATHINFO_EXTENSION));

			$response = $this->Estilos->save($estilo);
			...
			if ($response) {
				...
				$image->crop(new Point(
					...
					->save(WWW_ROOT . 'content' . DS . 'estilos' . DS . $estilo->imagem);
	...
	public function editar($id = null) {
		...
		if ($this->request->is('ajax')) {
			...
			if ($response) {
				if ($this->request->getData('img') && $this->request->getData('img')->getError() == 0) {
					if (file_exists(WWW_ROOT . 'content' . DS . 'estilos' . DS . $estiloOriginal->imagem)) {
						unlink(WWW_ROOT . 'content' . DS . 'estilos' . DS . $estiloOriginal->imagem);
					}
	...
	public function visibilidade($id = null) {
		if ($this->request->is(['ajax'])) {
			...
			$alternancia = $this->Estilos->query()
				->update()
				->set([
