var page_actions = function(){
	...
	$("html").on("click", function(event) {
		...
		if (!$(event.target).closest(".x-navigation-horizontal li,.x-navigation-minimized li,.x-features .x-features-search,.x-features .x-features-profile").length) {
			$(".x-navigation-horizontal li,.x-navigation-minimized li,.x-features .x-features-search,.x-features .x-features-profile").removeClass('active');
		}
	});
	...
	$(".x-navigation li").click(function(event) {
		if (!$(event.target).closest('.mb-control').length) {
			...
			if(li.children("ul").length > 0 || li.children(".panel").length > 0 || $(this).hasClass("xn-profile") > 0){
				...
				if($(this).hasClass("xn-profile").length > 0)
					return true;
				else
					return false;
			}
